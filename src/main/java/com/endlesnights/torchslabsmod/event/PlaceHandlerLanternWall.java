...
private static final HashMap<ResourceLocation,Block> PLACE_ENTRIES = new HashMap<>();
	...
	{		
				...
				&& (world.isAirBlock(placeAt) || world.getFluidState(placeAt).getFluid() == Fluids.WATER || world.getFluidState(placeAt).getFluid() == Fluids.FLOWING_WATER) )
		{
			...
			{
				world.setBlockState(placeAt, block.getDefaultState().with(HorizontalBlock.HORIZONTAL_FACING, face).with(BlockWallTorchSlab.HALF, Half.BOTTOM));
			}
			else
			{
				world.setBlockState(placeAt, block.getDefaultState().with(HorizontalBlock.HORIZONTAL_FACING, face).with(BlockWallTorchSlab.HALF, Half.TOP));
			}
